...
class EmployeesAddForm extends Component {
    ...
    onValueChange = (e) => {//Будет принимать аргумент объекта события, а затем изменять состояние
        this.setState({//Можно и через коллбэк функцию
            ...
            [e.target.name]: e.target.value //Так как к разным данным атрибута будет только одно обращение, нужно создать один атрибут, от которого будем отталкиваться, в нашем случае name
        }) 
    }

    onSubmit = (e) => {//Создание самого сотрудника при нажатии на кнопку
        e.preventDefault();
        if (this.state.name.length < 3 || !this.state.salary) return;//Простая проверка на заполнение
